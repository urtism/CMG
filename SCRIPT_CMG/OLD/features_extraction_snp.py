...
def print_var_snp_reduced(dictionary):
	...
	print '\t'.join(["CHROM","POS","REF","ALT","CallMutect","CallVarscan","CallVardict",
			...
			"STATUS_Vardict","SHIFT3_Vardict","MSI_Vardict","MSILEN_Vardict","SOR_Vardict",
	...
	for variante in dictionary.keys():
		...
		print '\t'.join([variante,str(features.CallMutect),str(features.CallVarscan),str(features.CallVardict),
			...
			str(features.STATUS_Vardict),str(features.SHIFT3_Vardict),str(features.MSI_Vardict),str(features.MSILEN_Vardict),str(features.SOR_Vardict),
...
def main():
	...
	callers = [opts.mutect,opts.varscan_snp,opts.varscan_snp,opts.vardict]
	varianti = dict() 
	index=0;
	for vcf in callers:
		#print vcf
